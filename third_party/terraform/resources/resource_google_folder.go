...
func resourceGoogleFolderCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = resourceManagerOperationWaitTime(config, op, nil, "creating folder", int(d.Timeout(schema.TimeoutCreate).Minutes()))
...
func resourceGoogleFolderUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    err = resourceManagerOperationWaitTime(config, op, nil, "move folder", int(d.Timeout(schema.TimeoutCreate).Minutes()))
		if err != nil {
			return fmt.Errorf("Error moving folder '%s' to '%s': %s", displayName, newParent, err)
		}
