...
type healthcareConsentStoreId struct {
	DatasetId healthcareDatasetId
	Name      string
}

func (s *healthcareConsentStoreId) consentStoreId() string {
	return fmt.Sprintf("%s/consentStores/%s", s.DatasetId.datasetId(), s.Name)
}

func (s *healthcareConsentStoreId) terraformId() string {
	return fmt.Sprintf("%s/%s", s.DatasetId.terraformId(), s.Name)
}

func parseHealthcareConsentStoreId(id string, config *Config) (*healthcareConsentStoreId, error) {
	parts := strings.Split(id, "/")

	consentStoreIdRegex := regexp.MustCompile("^(" + ProjectRegex + ")/([a-z0-9-])+/([a-zA-Z0-9_-]{1,256})/([a-zA-Z0-9_-]{1,256})$")
	consentStoreIdWithoutProjectRegex := regexp.MustCompile("^([a-z0-9-])+/([a-zA-Z0-9_-]{1,256})/([a-zA-Z0-9_-]{1,256})$")
	consentStoreRelativeLinkRegex := regexp.MustCompile("^projects/(" + ProjectRegex + ")/locations/([a-z0-9-]+)/datasets/([a-zA-Z0-9_-]{1,256})/consentStores/([a-zA-Z0-9_-]{1,256})$")

	if consentStoreIdRegex.MatchString(id) {
		return &healthcareConsentStoreId{
			DatasetId: healthcareDatasetId{
				Project:  parts[0],
				Location: parts[1],
				Name:     parts[2],
	...
	if consentStoreIdWithoutProjectRegex.MatchString(id) {
		if config.Project == "" {
			return nil, fmt.Errorf("The default project for the provider must be set when using the `{location}/{datasetName}/{consentStoreName}` id format.")
		}
	...
	if parts := consentStoreRelativeLinkRegex.FindStringSubmatch(id); parts != nil {
		return &healthcareConsentStoreId{
			DatasetId: healthcareDatasetId{
				Project:  parts[1],
				Location: parts[2],
	...
	return nil, fmt.Errorf("Invalid ConsentStore id format, expecting `{projectId}/{locationId}/{datasetName}/{consentStoreName}` or `{locationId}/{datasetName}/{consentStoreName}.`")
}
