...
// .hostname -> .hostname
// .labels -> .labels
// .metadata_startup_script -> .metadata.startup-script (verify)
// .project -> ?
func GetComputeInstanceApiObject(d TerraformResourceData, config *Config) (map[string]interface{}, error) {
	...
	// TODO: Draw from tf .boot_disk, .attached_disk, .scratch_disk
	...
	guestAcceleratorsProp, err := expandComputeInstanceGuestAccelerators(d.Get("guest_accelerator"), d, config)
	if err != nil {
		return nil, err
	} else if v, ok := d.GetOkExists("guest_accelerator"); !isEmptyValue(reflect.ValueOf(guestAcceleratorsProp)) && (ok || !reflect.DeepEqual(v, guestAcceleratorsProp)) {
	...
	networkInterfacesProp, err := expandComputeInstanceNetworkInterfaces(d.Get("network_interface"), d, config)
	if err != nil {
		return nil, err
	} else if v, ok := d.GetOkExists("network_interface"); !isEmptyValue(reflect.ValueOf(networkInterfacesProp)) && (ok || !reflect.DeepEqual(v, networkInterfacesProp)) {
	...
	serviceAccountsProp, err := expandComputeInstanceServiceAccounts(d.Get("service_account"), d, config)
	if err != nil {
		return nil, err
	} else if v, ok := d.GetOkExists("service_account"); !isEmptyValue(reflect.ValueOf(serviceAccountsProp)) && (ok || !reflect.DeepEqual(v, serviceAccountsProp)) {
...
func expandComputeInstanceGuestAccelerators(v interface{}, d TerraformResourceData, config *Config) (interface{}, error) {
	...
	for _, raw := range l {
		...
		transformedAcceleratorCount, err := expandComputeInstanceGuestAcceleratorsAcceleratorCount(original["count"], d, config)
		if err != nil {
			return nil, err
		...
		transformedAcceleratorType, err := expandComputeInstanceGuestAcceleratorsAcceleratorType(original["type"], d, config)
		if err != nil {
			return nil, err
...
// TODO: Switch return type from map[string]string to:
// struct {
//		fingerprint string
//		items []struct{
//			key		string
...
// TODO: tf -> gcloud
// [missing-in-docs] -> .kind
// [missing-in-docs] -> .fingerprint
func expandComputeInstanceNetworkInterfaces(v interface{}, d TerraformResourceData, config *Config) (interface{}, error) {
	...
	for _, raw := range l {
		...
		transformedAccessConfigs, err := expandComputeInstanceNetworkInterfacesAccessConfigs(original["access_config"], d, config)
		if err != nil {
			return nil, err
		...
		transformedAliasIpRanges, err := expandComputeInstanceNetworkInterfacesAliasIpRanges(original["alias_ip_range"], d, config)
		if err != nil {
			return nil, err
...
// TODO: tf -> gcloud
// .public_ptr_domain_name -> .publicPtrDomainName / .setPublicPtr
...
// TODO: tf -> gcloud
// [missing-in-docs] -> .nodeAffinities (object)
