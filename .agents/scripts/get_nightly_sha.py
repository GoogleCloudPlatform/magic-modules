...
def get_project_id():
    try:
        repo_name_output = subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, check=True).stdout.strip()
        repo_basename = os.path.basename(repo_name_output)
    except Exception:
        repo_basename = ""
    if repo_basename.endswith("-beta"):
...
def query_teamcity(sha_short, token):
    ...
    url = f"https://hashicorp.teamcity.com/app/rest/builds?locator=project:(id:{project_id}),branch:(name:refs/heads/nightly-test),number:{sha_short},count:1000&fields=build(id,status,state,finishDate,webUrl)"
    ...
    if not builds:
        return {
            ...
            "finish_date": "UNKNOWN",
            "web_url": f"https://hashicorp.teamcity.com/project/{project_id}?branch_{project_id}=refs%2Fheads%2Fnightly-test"
        }
    ...
    return {
        ...
        "compute_web_url": compute_web_url if compute_web_url else f"https://hashicorp.teamcity.com/project/{project_id}?branch_{project_id}=refs%2Fheads%2Fnightly-test"
    }
