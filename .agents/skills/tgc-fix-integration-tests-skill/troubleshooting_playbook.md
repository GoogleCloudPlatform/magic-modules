...
- **Solution**: 
  - **If resources differ by URL structure** (e.g., one has `regions/{{region}}` and the other has `zones/{{zone}}`): Group them by setting identical `cai_resource_kind` values in their Magic Modules YAML files. This allows the generator to automatically produce the `strings.Contains` dispatch logic based on URL paths in `convert_resource.go`.
  - **If resources differ by data payload**: Inject a manual type-checking override block directly into the `ConvertResource` function within `mmv1/templates/tgc_next/cai2hcl/convert_resource.go.tmpl` to explicitly handle the problematic asset type.
- **Example**:
...
#### 17. Handling Group Constraints (`exactly_one_of`, `at_least_one_of`, `required_with`)
- **Symptom**: `Invalid combination of arguments` or `Required field is not set` errors during plan.
- **Cause**: The API returns data that doesn't satisfy the mutual exclusivity or dependency rules in the Terraform schema, or CAI data is missing fields needed to satisfy the group.
- **Solution**: Use a `tgc_decoder` to inspect the resolved data and inject fallback values or enforce exclusivity.
  - For `exactly_one_of`: If neither is present, inject a dummy value for the preferred one. If both are present, unset the lower priority one.
  - For `required_with`: If one field is present, ensure ALL fields in the group are populated (use dummy values if necessary).

> [!TIP]
> **Common Dummy Values for Schema Satisfaction**:
> When forced to inject data that CAI doesn't provide to satisfy required fields or constraints:
> - **Strings**: `"unknown"`
> - **Integers**: `0`
> - **Booleans**: `false`
> - **Objects/Maps**: `{}`

#### 18. Panic: Slice Bounds Out of Range in ParseUrlParamValuesFromAssetName
