...
#
# Usage:
#   ./.agents/skills/utils/run-acctests/scripts/run_acctests.sh <SERVICE> [TEST_NAME]
#
# Examples:
...
if [ -z "$SERVICE" ]; then
  ...
  echo "Usage: $0 <service> [test_name_pattern]"
  exit 1
fi

if [[ "$SERVICE" == *"/"* ]] || [[ "$SERVICE" == "."* ]]; then
  echo -e "${RED}Error: First argument must be a service name (e.g. 'compute'), not a path.${NC}"
  exit 1
fi
...
run_acctest_for_version() {
  ...
  local SERVICE_PATH="./${FOLDER_NAME}/services/${SERVICE}"
