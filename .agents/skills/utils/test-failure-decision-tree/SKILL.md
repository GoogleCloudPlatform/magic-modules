...
* **Inspection:** Check for GCP backend crashes, quota exhaustion (`RESOURCE_EXHAUSTED` / `Quota limit exceeded`), or internal errors (`HTTP 500/502/503`). **Note:** Do NOT classify HTTP 403 `SERVICE_DISABLED` or API Not Enabled errors as Scenario 5; those must be classified as **Scenario 9 (API Service Not Enabled)** for automated remediation.
...
### Scenario 9: API Service Not Enabled (SERVICE_DISABLED / HTTP 403)
