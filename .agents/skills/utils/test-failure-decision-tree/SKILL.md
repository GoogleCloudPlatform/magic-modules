...
| **Scenario 5A (Generated Tests)** | Resource name collision (`alreadyExists`) or static string reuse in autogenerated tests | Sample HCL Template Naming Bug | Edit `mmv1/templates/terraform/samples/<service>/<sample>.tf.tmpl` to use `{{index $.ResourceIdVars "var_name"}}` and define `resource_id_vars` in product YAML |
| **Scenario 5B (Handwritten Tests)** | Resource name collision (`alreadyExists`) or static string reuse in handwritten Go tests | Handwritten Test Config Setup Bug | Edit `mmv1/third_party/terraform/services/<service>/<resource>_test.go.tmpl` to use dynamic random string generators (`fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))` or `%{random_suffix}`) |
...
* **Inspection:** Check for GCP backend crashes, quota exhaustion (`RESOURCE_EXHAUSTED` / `Quota limit exceeded`), or internal errors (`HTTP 500/502/503`). **Note:** Do NOT classify HTTP 403 `SERVICE_DISABLED` or API Not Enabled errors as Scenario 1; those must be classified as **Scenario 9 (API Service Not Enabled)** for automated remediation.
...
### Scenario 5A: Generated Test Naming & Setup (Autogenerated Tests)
...
### Scenario 5B: Handwritten Test Naming & Setup (Go Test Code)
