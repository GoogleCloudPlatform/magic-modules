...
This CLI configuration file you created in the steps above will allow Terraform to use locally-built versions of `terraform-provider-google` and `terraform-provider-google-beta`, while also continuing to source all other providers from the public Registry.
