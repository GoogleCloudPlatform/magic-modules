...
2. Update the [documentation for the field]({{< ref "/document/add-documentation" >}}) to include the deprecation notice. For example:
   ...
   ```markdown
   * `api_field_name` - (Optional, [Beta](https://registry.terraform.io/providers/hashicorp/google/latest/docs/guides/provider_versions), Deprecated) FIELD_DESCRIPTION. `api_field_name` is deprecated and will be removed in a future major release. Use `other_field_name` instead.
   ```
{{< /tab >}}
{{% /tabs %}}
