...
var fieldRule_OptionalComputedToOptional = FieldRule{
	...
	definition:  "A field cannot go from computed + optional to optional. On a sequential applies the terraform state will have the previously computed value. The value won't be present in the config, thus ultimately causing a diff.",
