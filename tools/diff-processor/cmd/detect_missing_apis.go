...
const detectMissingAPIsDesc = "Run the missing test detector using the given services directory"
...
func (o *detectMissingAPIsOptions) run(args []string) error {
	fileName := args[0]
	missingAPIs, err := detector.DetectMissingAPIs(o.computeSchemaDiff(), fileName)
	if err != nil {
