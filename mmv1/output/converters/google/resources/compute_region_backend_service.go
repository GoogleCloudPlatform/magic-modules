...
func NewComputeRegionBackendServiceConverter(name string) Converter {
...
func (c *ComputeRegionBackendServiceConverter) Convert(assets []*caiasset.Asset) ([]*HCLResourceBlock, error) {
...
func (c *ComputeRegionBackendServiceConverter) convertResourceData(asset *caiasset.Asset) (*HCLResourceBlock, error) {
	if asset == nil || asset.Resource == nil || asset.Resource.Data == nil {
		return nil, fmt.Errorf("asset resource data is nil")
	}

	var resource *compute.RegionBackendService
	if err := decodeJSON(asset.Resource.Data, &resource); err != nil {
		return nil, err
	}

	hcl, _ := resourceComputeRegionBackendServiceRead(resource)
...
func resourceComputeRegionBackendServiceRead(res *compute.RegionBackendService) (map[string]interface{}, error) {
	result := make(map[string]interface{})
	config := meta.(*transport_tpg.Config)

	result["affinity_cookie_ttl_sec"] = flattenComputeRegionBackendServiceAffinityCookieTtlSec(res.AffinityCookieTtlSec, config)
	result["backend"] = flattenComputeRegionBackendServiceBackend(res.Backend, config)
	result["circuit_breakers"] = flattenComputeRegionBackendServiceCircuitBreakers(res.CircuitBreakers, config)
	result["consistent_hash"] = flattenComputeRegionBackendServiceConsistentHash(res.ConsistentHash, config)
	result["cdn_policy"] = flattenComputeRegionBackendServiceCdnPolicy(res.CdnPolicy, config)
	if flattenedProp := flattenComputeRegionBackendServiceConnectionDraining(res.ConnectionDraining); flattenedProp != nil {
	...
	result["creation_timestamp"] = flattenComputeRegionBackendServiceCreationTimestamp(res.CreationTimestamp, config)
	result["description"] = flattenComputeRegionBackendServiceDescription(res.Description, config)
	result["failover_policy"] = flattenComputeRegionBackendServiceFailoverPolicy(res.FailoverPolicy, config)
	result["enable_cdn"] = flattenComputeRegionBackendServiceEnableCDN(res.EnableCDN, config)
	result["fingerprint"] = flattenComputeRegionBackendServiceFingerprint(res.Fingerprint, config)
	result["health_checks"] = flattenComputeRegionBackendServiceHealthChecks(res.HealthChecks, config)
	result["iap"] = flattenComputeRegionBackendServiceIap(res.Iap, config)
	result["load_balancing_scheme"] = flattenComputeRegionBackendServiceLoadBalancingScheme(res.LoadBalancingScheme, config)
	result["locality_lb_policy"] = flattenComputeRegionBackendServiceLocalityLbPolicy(res.LocalityLbPolicy, config)
	result["name"] = flattenComputeRegionBackendServiceName(res.Name, config)
	result["outlier_detection"] = flattenComputeRegionBackendServiceOutlierDetection(res.OutlierDetection, config)
	result["port_name"] = flattenComputeRegionBackendServicePortName(res.PortName, config)
	result["protocol"] = flattenComputeRegionBackendServiceProtocol(res.Protocol, config)
	result["session_affinity"] = flattenComputeRegionBackendServiceSessionAffinity(res.SessionAffinity, config)
	result["connection_tracking_policy"] = flattenComputeRegionBackendServiceConnectionTrackingPolicy(res.ConnectionTrackingPolicy, config)
	result["timeout_sec"] = flattenComputeRegionBackendServiceTimeoutSec(res.TimeoutSec, config)
	result["log_config"] = flattenComputeRegionBackendServiceLogConfig(res.LogConfig, config)
	result["network"] = flattenComputeRegionBackendServiceNetwork(res.Network, config)
	result["subsetting"] = flattenComputeRegionBackendServiceSubsetting(res.Subsetting, config)
	result["region"] = flattenComputeRegionBackendServiceRegion(res.Region, config)
