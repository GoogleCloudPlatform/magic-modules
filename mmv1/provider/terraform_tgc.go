...
func NewTerraformGoogleConversion(product *api.Product, versionName string, startTime time.Time, templateFS embed.FS) TerraformGoogleConversion {
...
func retrieveFullManifestOfNonDefinedTests(fs embed.FS) []string {
	var tests []string
...
func retrieveFullListOfTestFiles(fs embed.FS) []string {
	var testFiles []string

	files, err := fs.ReadDir("third_party/tgc/tests/data")
	if err != nil {
		log.Fatal(err)
	}
	for _, file := range files {
...
func retrieveFullListOfTestTilesWithLocation(fs embed.FS) map[string]string {
...
func retrieveTestSourceCodeWithLocation(fs embed.FS, suffix string) map[string]string {
...
func retrieveListOfManuallyDefinedTests(fs embed.FS) []string {
...
func retrieveListOfManuallyDefinedTestsFromFile(fs embed.FS, file string) []string {
