...
func NewTerraformGoogleConversionNext(product *api.Product, versionName string, startTime time.Time, templateFS embed.FS) TerraformGoogleConversionNext {
...
func (tgc TerraformGoogleConversionNext) CopyFileList(outputFolder string, files map[string]string) {
	for target, source := range files {
		...
		sourceByte, err := tgc.templateFS.ReadFile(source)
		if err != nil {
...
func (tgc TerraformGoogleConversionNext) addTestsFromHandwrittenTests(object *api.Resource) error {
	...
	data, err := tgc.templateFS.ReadFile(handwrittenTestFilePath)
	for err != nil {
