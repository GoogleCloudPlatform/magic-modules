...
func (t Terraform) addCopyfileHeader(srcpath, outputFolder, target string) {
	...
	if strings.Contains(srcStr, "***     AUTO GENERATED CODE    ***    Type: MMv1     ***") {
		return
	}
...
// ----------------------------------------------------------------------------` + "\n"
	if filepath.Ext(target) == ".markdown" {
		// for markdown, the header should be within the frontformatter section
		srcStr = strings.TrimPrefix(srcStr, "---\n")
		headerFormat = "---\n" + strings.Replace(headerFormat, "//", "#", -1)
	}
	sourceByte = google.Concat([]byte(fmt.Sprintf(headerFormat, srcpath)), []byte(srcStr))

	// format the go file
	if filepath.Ext(target) == ".go" {
		sourceByte, _ = format.Source(sourceByte)
	}
