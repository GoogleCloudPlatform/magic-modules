...
func sqlAdminOperationWaitTime(config *Config, res interface{}, project, quotaProject, activity, userAgent string, timeout time.Duration) error {
	op := &sqladmin.Operation{}
	err := Convert(res, op)
	if err != nil {
		return err
	...
	w := &SqlAdminOperationWaiter{
		Service: config.NewSqlAdminClient(userAgent, quotaProject),
		Op:      op,
		Project: project,
	}
