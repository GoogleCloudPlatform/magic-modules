...
func TestAccDataprocSessionTemplate_update(t *testing.T) {
	...
	context := map[string]interface{}{
		...
		"kms_key_name":    kms.BootstrapKMSKeyWithPurposeInLocationAndName(t, "ENCRYPT_DECRYPT", "us-west1", "tf-bootstrap-dataproc-session-template-key1").CryptoKey.Name,
...
resource "google_dataproc_session_template" "dataproc_session_templates_jupyter_update" {
    name     = "projects/%{project_name}/locations/us-west1/sessionTemplates/tf-test-jupyter-session-template%{random_suffix}"
    location = "us-west1"
...
resource "google_dataproc_metastore_service" "ms" {
  ...
  network_config {
    consumers {
      subnetwork = "projects/%{project_name}/regions/us-west1/subnetworks/%{subnetwork_name}"
    }
  }
}
`, context)
