...
func TestAccRedisCluster_redisClusterMaintenanceVersion(t *testing.T) {
	...
	// Use a reference map to inject the discovered version dynamically into Steps 3 and 4
	var targetVars = make(config.Variables)
	targetVars["target_version"] = config.StringVariable("dummy") // Default placeholder

	var skipUpdate bool
	var effVer string
	var allAvailVers []string
		...
		Steps: []resource.TestStep{
			{
				...
				// Step 1 Check: Read the actual versions from the deployed cluster
				Check: resource.ComposeTestCheckFunc(
					func(s *terraform.State) error {
						res, ok := s.RootModule().Resources["google_redis_cluster.cluster-ms"]
						if !ok {
							return fmt.Errorf("resource google_redis_cluster.cluster-ms not found in state")
						}
						attrs := res.Primary.Attributes
						effVer = attrs["effective_maintenance_version"]
						if effVer == "" {
							t.Logf("Warning: effective_maintenance_version is empty in state")
							skipUpdate = true
							return nil
						}

						availCountStr, ok := attrs["available_maintenance_versions.#"]
						if !ok || availCountStr == "" {
							t.Logf("No available_maintenance_versions found in state")
							skipUpdate = true
							return nil
						}

						availCount, err := strconv.Atoi(availCountStr)
						if err != nil {
							t.Logf("Warning: failed to parse available_maintenance_versions count: %v", err)
							skipUpdate = true
							return nil
						}

						allAvailVers = make([]string, 0, availCount)
						for i := 0; i < availCount; i++ {
							v := attrs[fmt.Sprintf("available_maintenance_versions.%d", i)]
							if v != "" {
								allAvailVers = append(allAvailVers, v)
								// Versions are strings like "REDISCLUSTER_YYYYMMDD_XX_XX".
								// Find the highest available version strictly newer than current
								if v > effVer {
						...
						if targetVersion != "" {
							// Update the variable value for Step 3 and Step 4
							targetVars["target_version"] = config.StringVariable(targetVersion)
							skipUpdate = false
							t.Logf("Found newer maintenance version %q to upgrade from %q. Enabling Step 3 and 4.", targetVersion, effVer)
						} else {
							t.Logf("No maintenance version in %v is newer than effective version %q. Skipping update steps.", allAvailVers, effVer)
							skipUpdate = true
						}
						return nil
					},
			...
			},
			{
				...
				ConfigVariables: targetVars, // Injects the discovered version
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PreApply: []plancheck.PlanCheck{
						plancheck.ExpectResourceAction("google_redis_cluster.cluster-ms", plancheck.ResourceActionUpdate),
					},
				},
				// Skip if no newer version was found in Step 1
				SkipFunc: func() (bool, error) {
					if skipUpdate {
						t.Logf("Skipping maintenance version update test: No newer maintenance version available (Current: %q, Available: %v)", effVer, allAvailVers)
						return true, nil
					}
					return false, nil
				},
			},
			{
				...
				ImportState:       true,
				ImportStateVerify: true,
				ConfigVariables:   targetVars,
				// Skip if update was skipped
				SkipFunc: func() (bool, error) {
					if skipUpdate {
						t.Logf("Skipping maintenance version verify step due to skipped update")
						return true, nil
					}
					return false, nil
				},
...
resource "google_redis_cluster" "cluster-ms" {
  ...
  transit_encryption_mode 	 = "TRANSIT_ENCRYPTION_MODE_SERVER_AUTHENTICATION"
  authorization_mode 		 = "AUTH_MODE_DISABLED"
  maintenance_version 		 = var.target_version
  redis_configs = { 
    maxmemory-policy		 = "volatile-ttl"
  }
  deletion_protection_enabled = false
