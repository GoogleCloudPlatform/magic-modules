...
func TestValidateIAMWorkforcePoolWorkforcePoolProviderKeyId(t *testing.T) {
	...
	es := verify.TestStringValidationCases(x, iamworkforcepool.ValidateWorkforcePoolProviderKeyId)
	if len(es) > 0 {
		t.Errorf("Failed to validate WorkforcePoolProviderKey names: %v", es)
	}
}
