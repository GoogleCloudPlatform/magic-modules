...
func TestAccDeploymentManagerDeployment_basicFile(t *testing.T) {
	...
	yamlPath := createYamlConfigFileForTest(t, acctest.TestFixturesPath+"/deploymentmanager/service_account.yml.tmpl", map[string]interface{}{
...
func TestAccDeploymentManagerDeployment_imports(t *testing.T) {
	...
	deploymentName := "tf-dm-" + randStr
	accountId := "tf-dm-" + randStr
	importFilepath := createYamlConfigFileForTest(t, acctest.TestFixturesPath+"/deploymentmanager/service_account.yml.tmpl", map[string]interface{}{
		"account_id": "{{ env['name'] }}",
	})
