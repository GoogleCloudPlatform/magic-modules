...
func TestAccStorageInsightsReportConfig_reportTypeConflict(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccStorageInsightsReportConfig_reportTypeConflict(context),
				ExpectError: regexp.MustCompile("only one of `csv_options,parquet_options`"),
			},
		},
	})
}
...
func testAccStorageInsightsReportConfig_reportTypeConflict(context map[string]interface{}) string {
