		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					acctest.CheckTagBindings(t, testAccComputeStoragePoolTagBindingCheckConfig(
						"google_compute_storage_pool.test-storage-pool-with-resource-manager-tags",
			...
			{
				Config: testAccComputeStoragePool_resourceManagerTagsUpdated(context),
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PreApply: []plancheck.PlanCheck{
						plancheck.ExpectResourceAction("google_compute_storage_pool.test-storage-pool-with-resource-manager-tags", plancheck.ResourceActionReplace),
					},
				},
				Check: resource.ComposeTestCheckFunc(
					acctest.CheckTagBindings(t, testAccComputeStoragePoolTagBindingCheckConfig(
...
func testAccComputeStoragePool_resourceManagerTagsUpdated(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_tags_tag_key" "tag_key" {
  parent      = "projects/%{project_id}"
  short_name  = "storage-pool-tag-%{random_suffix}"
...
func testAccComputeStoragePoolTagBindingCheckConfig(resourceName string, expectedTagValueResources, unexpectedTagValueResources []string) acctest.TagBindingCheckConfig {
	return acctest.TagBindingCheckConfig{
		...
		GetLocation: func(rs *terraform.ResourceState) (string, error) {
			zone := rs.Primary.Attributes["zone"]
			if zone == "" {
				return "", fmt.Errorf("expected zone to be set for %s", resourceName)
			}
			return zone, nil
		},
	}
