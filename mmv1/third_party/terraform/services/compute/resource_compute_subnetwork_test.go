...
func testAccCheckComputeSubnetworkHasNotSecondaryIpRange(subnetwork *map[string]interface{}, rangeName, ipCidrRange string) resource.TestCheckFunc { 60988a8e7 (Migrate resource_compute_subnetwork_test.go.tmpl to new API):mmv1/third_party/terraform/services/compute/resource_compute_subnetwork_test.go
	return func(s *terraform.State) error {
		if ranges, ok := (*subnetwork)["secondaryIpRanges"].([]interface{}); ok {
			for _, raw := range ranges {
				r := raw.(map[string]interface{})
