...
func TestAccComputePreviewFeature_update(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Enable the "alpha-api-access" feature and verify its attributes.
			{
				Config: testAccComputePreviewFeature_enable(featureName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", featureName),
					resource.TestCheckResourceAttr(resourceName, "activation_status", "ENABLED"),
				),
			},
			...
			// Step 3: Disable the "alpha-api-access" feature and verify its attributes.
			{
			...
			// Step 4: Verify that the resource can be successfully imported.
			{
				ResourceName:            resourceName,
				ImportState:             true,
				ImportStateVerify:       true,
...
func testAccComputePreviewFeature_enable(name string) string {
	return fmt.Sprintf(`
