	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_vpc_access_connector.connector", "name", fmt.Sprintf("tf-test-immutable%s", context["random_suffix"])),
				),
			},
			{
				ResourceName:      "google_vpc_access_connector.connector",
...
func testAccVPCAccessConnector_vpcAccessConnectorUpdateImmutableField(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_vpc_access_connector" "connector" {
  name          = "tf-test-immutable%{random_suffix}"
  subnet {
    name = google_compute_subnetwork.custom_test.name
  }
  machine_type = "e2-standard-4"
