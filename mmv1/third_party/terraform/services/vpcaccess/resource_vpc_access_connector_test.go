...
func TestAccVPCAccessConnector_vpcAccessConnectorThroughput_maxThroughput(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVPCAccessConnector_vpcAccessConnectorThroughput_maxThroughput(context),
			},
			{
				ResourceName:      "google_vpc_access_connector.connector",
				ImportState:       true,
...
func testAccVPCAccessConnector_vpcAccessConnectorThroughput_maxThroughput(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_vpc_access_connector" "connector" {
  name          = "tf-test-vpc-con%{random_suffix}"
  subnet {
