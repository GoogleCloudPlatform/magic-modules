...
func TestAccStorageTransferJob_transferPathError(t *testing.T) {
	t.Parallel()
	...
	testTransferJobDescription := acctest.RandString(t, 10)
	testTransferJobName := fmt.Sprintf("tf-test-transfer-job-%s", acctest.RandString(t, 10))
	errorMessage := string('"') + "transfer_spec.0.gcs_data_sink.0.path" + string('"') + " cannot start with /"
...
func testAccStorageTransferJob_transferJobPathError(project string, dataSourceBucketName string, dataSinkBucketName string, transferJobDescription string, testTransferJobName string) string {
	return fmt.Sprintf(`
  data "google_storage_transfer_project_service_account" "default" {
    project = "%s"
  }
