...
func TestAccApigeeApi_space(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:            "google_apigee_api.test_apigee_api_space",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"config_bundle", "detect_md5hash", "md5hash"},
			},
...
resource "google_apigee_api" "test_apigee_api_space" {
  name            = "tf-test-apigee-api-space"
  org_id          = google_project.project.project_id
  space           = google_apigee_space.space.name
  config_bundle   = "./test-fixtures/apigee_api_bundle.zip"
  depends_on      = [google_apigee_space.space]
}
`, context)
}
