...
func testAccCheckMetastoreServiceHasTagBindings(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			// 1. Get the configured tag
			var configuredTagValueShortName string
			for key, val := range rs.Primary.Attributes {
				if strings.HasPrefix(key, "tags.") {
					configuredTagValueShortName = val
					break
				}
			}

			if configuredTagValueShortName == "" {
				return fmt.Errorf("could not find a configured tag value in the state for resource %s", rs.Primary.ID)
			}

			// 2. Describe the tag value to get its full resource name (e.g., "tagValues/123456").
			describeTagValueURL := fmt.Sprintf("https://cloudresourcemanager.googleapis.com/v3/%s", configuredTagValueShortName)
			...
			if err != nil {
				return fmt.Errorf("error describing tag value %s: %v", configuredTagValueShortName, err)
			}

			fullTagValueName, ok := respDescribe["name"].(string)
			if !ok || fullTagValueName == "" {
				return fmt.Errorf("tag value details not found in response for short name: %s", configuredTagValueShortName)
			}
			...
			if !foundMatch {
				return fmt.Errorf("expected tag value %s not found in tag bindings for resource %s", fullTagValueName, rs.Primary.ID)
			}

			t.Logf("successfully found matching tag binding for %s", rs.Primary.ID)
		}
