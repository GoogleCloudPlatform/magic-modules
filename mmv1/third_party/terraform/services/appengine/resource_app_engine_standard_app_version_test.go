...
func TestAccAppEngineStandardAppVersion_vpcAccess(t *testing.T) {
	t.Skip("https://github.com/hashicorp/terraform-provider-google/issues/18936")
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAppEngineStandardAppVersion_vpcAccess(context),
			},
			{
				ResourceName:            "google_app_engine_standard_app_version.foo",
				ImportState:             true,
...
func testAccAppEngineStandardAppVersion_vpcAccess(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_project" "my_project" {
  name = "tf-test-appeng-std%{random_suffix}"
  project_id = "tf-test-appeng-std%{random_suffix}"
