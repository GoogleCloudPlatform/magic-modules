		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile("[\\s\\S]*Please add these missing dependencies:[\\s\\S]*Added service: \\[\"services\\/compute\\.googleapis\\.com\"][\\s\\S]*"),
			},
			{
			...
			},
			{
				...
				ExpectError: regexp.MustCompile("[\\s\\S]*Please remove existing dependent services:[\\s\\S]*Removed service: \\[\"services\\/oslogin\\.googleapis\\.com\"][\\s\\S]*"),
			},
			{
...
resource "google_project" "basic" {
  provider = google-beta
  project_id = "suv2-test%{random_suffix}"
