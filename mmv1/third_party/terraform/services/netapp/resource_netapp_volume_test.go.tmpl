...
func TestAccNetappVolume_volumeExportPolicyWithSquashMode(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
            ...
            {
				Config: testAccNetappVolume_volumeExportPolicyWithSquashMode_noRootSquash_hasRootAccess(context),
			},
			{
				ResourceName:            "google_netapp_volume.test_volume",
				ImportState:             true,
...
func testAccNetappVolume_volumeExportPolicyWithSquashMode_noRootSquash_hasRootAccess(context map[string]interface{}) string {
	return acctest.Nprintf(`
  resource "google_netapp_storage_pool" "default" {
      provider = google-beta
      name = "tf-test-pool%{random_suffix}"
