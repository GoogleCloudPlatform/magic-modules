...
func TestAccNetappVolume_autoTieredNetappVolume(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNetappVolume_autoTieredVolumeBasicExample_basic(context),
			},
			{
				ResourceName:            "google_netapp_volume.test_volume",
				ImportState:             true,
...
func testAccNetappVolume_autoTieredVolumeBasicExample_basic(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_netapp_storage_pool" "default" {
    name = "tf-test-pool%{random_suffix}"
    location = "us-west2"
