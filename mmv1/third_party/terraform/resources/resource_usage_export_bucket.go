...
func resourceProjectUsageBucketCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(project)
	err = computeOperationWaitTime(d, config, op, project, "Setting usage export bucket.", userAgent, d.Timeout(schema.TimeoutCreate))
	if err != nil {
		d.SetId("")
		return err
	}
...
func resourceProjectUsageBucketDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = computeOperationWaitTime(d, config, op, project,
