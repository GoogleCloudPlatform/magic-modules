...
func resourceComputeSharedVpcHostProjectCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = computeOperationWaitTime(d, config, op, hostProject, "Enabling Shared VPC Host", userAgent, d.Timeout(schema.TimeoutCreate))
	if err != nil {
		d.SetId("")
		return err
	}
...
func resourceComputeSharedVpcHostProjectDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = computeOperationWaitTime(d, config, op, hostProject, "Disabling Shared VPC Host", userAgent, d.Timeout(schema.TimeoutDelete))
	if err != nil {
		return err
	}
