...
func TestAccStorageTransferJob_basic(t *testing.T) {
	...
	vcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccStorageTransferJob_logging(getTestProjectFromEnv(), testUpdatedDataSourceBucketName, testUpdatedDataSinkBucketName, testUpdatedTransferJobDescription),
			},
			{
				ResourceName:      "google_storage_transfer_job.transfer_job",
				ImportState:       true,
...
func testAccStorageTransferJob_logging(project string, dataSourceBucketName string, dataSinkBucketName string, transferJobDescription string) string {
	return fmt.Sprintf(`
data "google_storage_transfer_project_service_account" "default" {
  project = "%s"
}
...
resource "google_storage_transfer_job" "transfer_job" {
  ...
  transfer_spec {
    ...
    logging_config {
      ...
      log_actions_states = ["failed","succeeded"]
