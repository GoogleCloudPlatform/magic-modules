...
func testSweepFirebaseAppleApp(region string) error {
	...
	res, err := google.SendRequest(config, "GET", config.Project, listUrl, config.UserAgent, nil)
	if err != nil {
	...
	for _, ri := range rl {
		...
		_, err = google.SendRequest(config, "POST", config.Project, deleteUrl, config.UserAgent, body)
		if err != nil {
