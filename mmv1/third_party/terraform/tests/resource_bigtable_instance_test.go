...
func TestAccBigtableInstance_createWithAutoscalingAndNumNodesFails(t *testing.T) {
	...
	vcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccBigtableInstance_autoscalingClusterConfigWithNumNodes(instanceName, 2, 5, 70, 1),
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccBigtableInstance_autoscalingClusterConfigWithStorageTarget(instanceName, 2, 5, 70, 9000),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("google_bigtable_instance.instance", "cluster.0.num_nodes", "2"),
			...
			},
			{
				...
				Config: testAccBigtableInstance_autoscalingClusterConfigWithStorageTarget(instanceName, 1, 5, 80, 10000),
				Check: resource.ComposeTestCheckFunc(
...
func testAccBigtableInstance_autoscalingClusterConfigWithStorageTarget(instanceName string, min int, max int, cpuTarget int, storageTarget int) string {
	return fmt.Sprintf(`resource "google_bigtable_instance" "instance" {
		name = "%s"
		cluster {
			cluster_id   = "%s"
...
func testAccBigtableInstance_autoscalingClusterConfigWithNumNodes(instanceName string, min int, max int, cpuTarget int, numNodes int) string {
	return fmt.Sprintf(`resource "google_bigtable_instance" "instance" {
		name = "%s"
		cluster {
			cluster_id   = "%s"
