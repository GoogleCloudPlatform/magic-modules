...
Default labels configured on the provider through the new `default_labels` field are now supported. The default labels configured on the provider will be applied to all of the resources with the top level `labels` field or the nested `labels` field inside the top level `metadata` field. Labels with an empty value (`""`) should be avoided, as they will be ignored and not included in `terraform_labels`, `effective_labels` and then API requests. Instead, labels with the value `_` or `true` are recommended.

Provider-level default annotations are not supported.

#### Resource labels

Labels and annotations fields on Terraform google provider were authoritative and thought Terraform being the only owner of the fields. This model worked well at the beginning. But with the introduction of the service system labels and other clients managed labels, it shows diff of these service system labels and other clients managed labels when the user updates the `labels` or `annotations` fields with Terraform. However, the diff is unexpected because the user does not modify these system labels or other clients managed labels with Terraform. The new labels model is the solution to this problem.
...
* The `labels` field will be non-authoritative and only manage the labels defined by the users on the resource through Terraform. If a label was added outside of Terraform, it will not be managed by Terraform, unless it is added to the `labels` field in the configuration. The out of band labels will be listed in the `effective_labels` field. Also, the labels with an empty value (`""`) should be avoided, as they will be ignored and not included in `terraform_labels`, `effective_labels` and then API requests. Instead, labels with the value `_` or `true` are recommended.
...
**Note:** `ignore_changes` can be applied to `labels` field to ignore the changes of the user defined labels. It is not recommended to apply `ignore_changes` to `terraform_labels` or `effective_labels`, as it will make the new labels model not work as expected.

The following changes will be observed after upgrading to `5.0.0`.
* After upgrading to `5.0.0`, and then running `terraform import`, these three fields should show in the state file of the resources with empty `labels` and empty `terraform_labels`. `effective_labels` will have all of labels present on the resource in GCP.
...
* In the resource `google_cloud_run_domain_mapping`, the system labels `cloud.googleapis.com/location` and `run.googleapis.com/overrideAt` are removed from `labels` inside `metadata` field in the state file. If any of these label keys are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.
* In the resource `google_cloud_run_service`, the system label `cloud.googleapis.com/location` is removed from `labels` inside `metadata` field in the state file. If this system label key is in the configration, after upgrading to `5.0.0`, the plan shows that this key will be added.
* In the resource `google_dataflow_flex_template_job`, the system labels with the prefix `goog-dataflow-provided` are removed from the `labels` field in the state file. If any of these system label keys are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.
* In the resource `google_dataflow_job`, the system labels with the prefix `goog-dataflow-provided` are removed from the `labels` field in the state file. If any of these system label keys are in the configration, after upgrading to `5.0.0`, the plan shows that the keys will be added.
* In the resource `google_dataproc_cluster`, the system labels with the prefix `goog-dataproc` are removed from the `labels` field in the state file. If any of these system label keys are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.
* In the resource `google_notebooks_instance`, the system labels with the prefix `goog-caip-notebook` are removed from the `labels` field in the state file. If any of these system label keys are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.
* In the resource `google_storage_bucket`, the system labels with the prefix `goog-dataplex` are removed from the `labels` field in the state file. If any of these system label keys are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.

#### Data source labels

Since the resource has used the new labels model, the corresponding data source now has three label-related fields as well, `labels`, `effective_labels` and `terraform_labels`. All of these three fields will have all of labels present on the resource in GCP, including the labels configured through Terraform, other clients and services.
...
**Note:** `ignore_changes` can be applied to `annotations` field to ignore the changes of the user defined annotations. It is not recommended to apply `ignore_changes` to `effective_annotations`, as it will make the new annotations model not work as expected.
...
* After upgrading to `5.0.0`, and then running `terraform refresh`, these two fields should show in the state file of the resources with a self-applying `annotations` field. `effective_annotations` will have all of annotations present on the resource in GCP.
* After upgrading to `5.0.0`, and then running `terraform plan` or `terraform apply`, these two fields should show in the state file of the resources with a self-applying `annotations` field. `effective_annotations` will have all of annotations present on the resource in GCP.
* In the resource `google_cloud_run_domain_mapping`, the system annotations `serving.knative.dev/creator`, `serving.knative.dev/lastModifier`, `run.googleapis.com/operation-id`, `run.googleapis.com/ingress-status`, `run.googleapis.com/ingress` with the value `all` are removed from `annotations` inside `metadata` field in the state file. If any of these system annotations are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.
* In the resource `google_cloud_run_service`, tthe system annotations `serving.knative.dev/creator`, `serving.knative.dev/lastModifier`, `run.googleapis.com/operation-id`, `run.googleapis.com/ingress-status`, `run.googleapis.com/ingress` with the value `all` are removed from `annotations` inside `metadata` field in the state file. If any of these system annotations are in the configration, after upgrading to `5.0.0`, the plan shows that these keys will be added.

#### Data source annotations

Since the resource has used the new annotations model, the corresponding data source now has two annotation-related fields as well, `annotations`, and `effective_annotations`. Both fields will have all of annotations present on the resource in GCP, including the annotations configured through Terraform, other clients and services.
