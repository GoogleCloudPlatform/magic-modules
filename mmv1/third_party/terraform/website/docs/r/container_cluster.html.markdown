...
*  `lustre_csi_driver_config` - (Optional) The status of the Lustre CSI driver addon,
   ...
   `enable_legacy_lustre_port` which allows the Lustre CSI driver to initialize LNet (the virtual network
   layer for Lustre kernel module) using port 6988. This flag is required to workaround a port conflict with the gke-metadata-server on GKE nodes.
<!-- 
TODO: Add Managed Lustre CSI Driver doc once available
-->
