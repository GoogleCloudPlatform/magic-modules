...
* `policy` - (Optional) The Topology Manager policy to use. This policy dictates how resource alignment is handled on the node.

* `scope` - (Optional) The Topology Manager scope, defining the granularity at which policy decisions are applied. Valid values are "container" (resources are aligned per container within a pod) or "pod" (resources are aligned for the entire pod).
...
* `policy` - (Optional) The [Memory Manager](https://kubernetes.io/docs/tasks/administer-cluster/memory-manager/) policy to use. This policy dictates how memory alignment is handled on the node.
