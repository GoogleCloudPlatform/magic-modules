...
* `resource_manager_tags` - (Optional) A map of resource manager tags, Keys must be in the format tagKeys/{tag_key_id} and values are in the format
  tagValues/{tag_key_id} (see [Secure tags](https://cloud.google.com/dataproc/docs/guides/use-secure-tags)).
