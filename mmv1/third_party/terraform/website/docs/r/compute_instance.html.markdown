...
* `machine_type` - (Required) The machine type to create.
    ...
    **Note:** If you want to update this value (resize the VM) after initial creation, you must set [`allow_stopping_for_update`](#allow_stopping_for_update) to `true`.
    ...
    If your custom `machine_type` is not covered by this syntax you will need to set `lifecycle_rule.ignore_changes` to `true`
    because most likely the Compute API will try to override your input.
