...
The `google_compute_instance_power` action enables power operations (`stop`, `start`, `restart`) on a Compute Engine instance. This feature uses Terraform Actions introduced in Terraform CLI v1.14+. For example:
