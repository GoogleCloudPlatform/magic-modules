...
* `provisioning_model` - (Optional) Describe the type of provisioning model for the instance. This field accepts the value `STANDARD`, `SPOT`, or `FLEX_START`. If the value is `STANDARD`, there will be no discount. If this is set to `SPOT`,
