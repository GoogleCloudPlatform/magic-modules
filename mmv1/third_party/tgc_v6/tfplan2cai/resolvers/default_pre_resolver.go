...
func (r *DefaultPreResolver) AddResourceChanges(changes []*tfjson.ResourceChange) map[string][]*models.FakeResourceData {
	...
	for _, rc := range changes {
		...
		if rc.ProviderName == "registry.terraform.io/hashicorp/google-beta" {
			r.errorLogger.Debug(fmt.Sprintf("%s: resource uses the google-beta provider and may not be convertible", rc.Address))
		}

		// Skip resources not found in the google GA provider's schema
		if _, ok := r.schema.ResourcesMap[rc.Type]; !ok {
			r.errorLogger.Debug(fmt.Sprintf("%s: resource type not found in google GA provider: %s.", rc.Address, rc.Type))
			continue
		}

		// Skip unsupported resources
		if _, ok := converters.ConverterMap[rc.Type]; !ok {
