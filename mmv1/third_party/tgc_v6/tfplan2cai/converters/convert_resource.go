...
func ConvertResource(rdList []*models.FakeResourceData, cfg *transport_tpg.Config, am ancestrymanager.AncestryManager, errLogger *zap.Logger) ([]caiasset.Asset, error) {
	...
	for _, rd := range rdList {
		...
		if !ok {
			errLogger.Debug(fmt.Sprintf("%s: resource type cannot be converted for CAI-based policies: %s. For details, see https://cloud.google.com/docs/terraform/policy-validation/create-cai-constraints#supported_resources", rd.TfplanAddr(), rd.Kind()))
			continue
		} else {
