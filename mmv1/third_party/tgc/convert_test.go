...
func TestConvertMergingRetryFetchFullResource(t *testing.T) {
	tests := []struct {
		name    string
		changes []*tfjson.ResourceChange
		want    []caiasset.Asset
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			cfg, err := resources.NewConfig(context.Background(), "", "", "", true, "", nil)
			assert.Nil(t, err)
			converter := NewConverter(cfg, &ancestrymanager.NoOpAncestryManager{}, false, false, zap.NewNop())

			fetchFuncCalled := 0
			fetchFunc := func(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (cai.Asset, error) {
				fetchFuncCalled++
				if fetchFuncCalled == 1 {
					return cai.Asset{}, io.ErrUnexpectedEOF
				}
				return cai.Asset{
					Name: "//cloudresourcemanager.googleapis.com/projects/example-project",
					Type: "cloudresourcemanager.googleapis.com/Project",
			...
			assert.Equal(t, 2, fetchFuncCalled)
		})
	}
}
