...
func TestPropertyLineage(t *testing.T) {
	t.Parallel()
	...
	flattenedRoot.SetDefault(&Resource{})
	...
	cases := []struct {
		...
		{
			description: "property in flattened object",
			obj:         *flattenedRoot.Properties[0],
			expected:    "payload.secretData",
		},
		{
			description: "property in array under flattened object",
			obj:         *flattenedRoot.Properties[1].ItemType.Properties[0],
			expected:    "payload.items.secretData",
		},
	}
	...
	for _, tc := range cases {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			got := strings.Join(tc.obj.PropertyLineage(), ".")
			if got != tc.expected {
				t.Errorf("expected %q to be %q", got, tc.expected)
			}
		})
