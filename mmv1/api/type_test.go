...
func TestGetPropertySchemaPath(t *testing.T) {
	...
	makeRes := func(props []*Type) *Resource {
		res := &Resource{
			Name:            "TestResource",
			ProductMetadata: &Product{Name: "TestProduct"},
			Properties:      props,
		}
		for _, p := range props {
			p.ResourceMetadata = res
		}
		return res
	}

	t.Run("resolves flattened path", func(t *testing.T) {
		t.Parallel()
		res := makeRes([]*Type{{
			Name:          "payload",
			Type:          "NestedObject",
	...
	t.Run("drops non-terminal repeated list segment", func(t *testing.T) {
		t.Parallel()
		res := makeRes([]*Type{{
			Name: "containers",
			Type: "Array",
		...
		got := res.Properties[0].GetPropertySchemaPath("containers.0.image")
		if got != "" {
			t.Errorf("expected empty path for repeated non-terminal list segment, got %q", got)
		}
	})

	t.Run("keeps terminal repeated list field", func(t *testing.T) {
		t.Parallel()
		res := makeRes([]*Type{{
			Name: "budgetFilter",
			Type: "NestedObject",
