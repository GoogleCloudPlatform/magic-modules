...
func (m *mockRunner) Run(path, name string, args, env []string) (string, string, error) {
	m.calledMethods["Run"] = append(m.calledMethods["Run"], []any{path, name, args, env})
	cmd := fmt.Sprintf("%s %s %v %v", path, name, args, env)
