...
# Find all product folders that contain a terraform.yaml because any 'product' with a terraform.yaml file
# present is capable of generating terraform code. 
TERRAFORM_PRODUCTS=`find products -name 'terraform.yaml'  -print0 | xargs -0 -n1 dirname | paste -sd "," -`

pushd magic-modules-branched
LAST_COMMIT_AUTHOR="$(git log --pretty="%an <%ae>" -n1 HEAD)"
bundle install
bundle exec compiler -p ${TERRAFORM_PRODUCTS} -e terraform -o "${GOPATH}/src/github.com/terraform-providers/terraform-provider-google/"
